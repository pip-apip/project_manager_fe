                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="form-group position-relative has-icon-left">
                                    ...
                                    <input type="text" class="form-control form-control-lg" id="tagInput"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="form-group" id="meet_of_person">
                    ...
                    <div class="col-md-12 text-center">
                        <button type="button" class="btn btn-secondary" id="openActivity">Buka Aktivitas</button>
    ...
    <script>
        ...
        function renderDocs(filteredDocs) {
            ...
            if (filteredDocs.length === 0) {
                $('#content_card').html(
                    '<div class="card card-body"><p class="card-text text-center">Dokumen tidak ditemukan</p></div>');
                return;
            }
            ...
            $.each(filteredDocs, function(index, doc) {
                ...
                $.each(doc.tags, function(indexs, tag) {
                    content_tag +=
                        `<span class="badge bg-light-secondary" style="margin:5px;">${tag}</span>`;
                });
                let date = doc.date ? doc.date : null;
                ...
                <div class="card" style="margin-bottom: 1.1rem;">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-5 d-md-flex justify-content-end d-none">
                                <small>${dateFormat(date) || 'Unknown Date'}</small>
                            </div>
                        </div>
                    </div>
                    <div class="card-body" style="padding-top:1px;">
                        ...
                        <div class="d-flex justify-content-end">
                            <small class="d-md-none mt-2">${dateFormat(doc.date) || 'Unknown Date'}</small>
        ...
        function readModal(id) {
            ...
            const docRouteBase = `{{ route('activity.doc', ['id' => ':id']) }}`;
            ...
            modal.find('#date').text(dateFormat(doc.date));
            modal.find('#location').text(doc.location || 'Tidak ada lokasi yang ditentukan');

            let meetOfPersonShow = `<ol>`;
            meetOfPersonShow += doc.meet_of_person.map(person => `<li>${person}</li>`).join("");
            meetOfPersonShow += `</ol>`;
            modal.find('#meet_of_person').html(meetOfPersonShow || 'Tidak ada peserta yang ditentukan');

            let agendaShow = `<ol>`;
            agendaShow += doc.agenda.map(agenda => `<li>${agenda}</li>`).join("");
            agendaShow += `</ol>`;
            modal.find('#agenda').html(agendaShow || 'Tidak ada agenda yang ditentukan');

            let activityShow = `<ol>`;
            activityShow += doc.activity.map(activity => `<li>${activity}</li>`).join("");
            activityShow += `</ol>`;
            modal.find('#activity').html(activityShow || 'Tidak ada aktivitas yang ditentukan');

            let tagsShow = doc.tags.map(tag =>
                `<button class="btn btn-info me-2">${tag}</button>`
            ).join("");
            modal.find('#tags').html(tagsShow);
            modal.find('#modalTitle').text('Document Activity - MOM (' + dateFormat(doc.date) + ')');
            modal.find('#openActivity').off('click').on('click', function() {
                let docRoute = docRouteBase.replace(':id', id);
                window.open(docRoute, '_blank');
            });
            modal.modal('show');
        }
