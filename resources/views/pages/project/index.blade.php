...
@section('content')
    <style>
        ...
        .scrollable-table thead th input[type="text"]:focus {
            ...
            box-shadow: none;
            border-color: #ccc;
            /* atau warna border default yang kamu mau */
        }
            ...
            <section class="section">
                <div class="card">
                                ...
                                <div class="col-sm-4 col-4 d-flex justify-content-end align-items-center">
                                    <a href="{{ route('project.create') }}" class="btn btn-success btn-sm">
                                        <i class="fa-solid fa-plus"></i> <span
                                            class="d-none d-md-inline-block">Tambah</span>
                                    </a>
                                </div>
                            @else
                            @endif
                    ...
                    <div class="card-body">
                        ...
                        @endphp
                        <div class="row">
                            <form method="GET" action="{{ route('project.index') }}" id="pagination-form"
                            ...
                            <form method="POST" action="{{ route('project.filter') }}" id="search-form"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-5 col-8">
                                        <div class="input-group">
                                            <input type="text" class="form-control" name="q"
                                                ...
                                                placeholder="Ketik Nama Proyek & Klik Enter ..."
                                    ...
                                    <div class="col-lg-1 col-3">
                                        <a href="{{ route('project.reset') }}" class="btn btn-secondary" type="button"
                                        ...
                                        @foreach ($results as $project)
                                            @php
                                                ...
                                                $project_leader_status =
                                                    session('user.id') == $project['project_leader_id'] ? true : false;
                                                $tooltip = $project_leader_status
                                                    ? 'Status = Proyek Leader'
                                                    : 'Status = Team Proyek';
                                                $tooltipAttr =
                                            ...
                                            <tr {!! $tooltipAttr !!}>
                                                ...
                                                <td class="text-center">
                                                    @if (session('user.role') == 'SUPERADMIN' || session('user.role') == 'ADMIN' || $project_leader_status)
                                                        <button class="badge {{ $badge_bg }} border-0"
                                                        ...
                                                        <div class="dropdown-menu" style="">
                                                            <button
                                                                class="dropdown-item {{ $project['status'] == 'WAITING' ? 'active' : '' }}"
                                                                onclick="changeStatus({{ $project['id'] }}, 'WAITING')">WAITING</button>
                                                            <button
                                                                class="dropdown-item {{ $project['status'] == 'ON PROGRESS' ? 'active' : '' }}"
                                                                onclick="changeStatus({{ $project['id'] }}, 'ON PROGRESS')">ON
                                                                PROGRESS</button>
                                                            <button
                                                                class="dropdown-item {{ $project['status'] == 'CLOSED' ? 'active' : '' }}"
                                                                onclick="changeStatus({{ $project['id'] }}, 'CLOSED')">CLOSED</button>
                                                        </div>
                                                    @else
                                                ...
                                                <td class="text-center">
                                                    <a onclick="showDetail({{ json_encode($project) }})"
                                                    ...
                                                    @endif
                                                    <a id="teamButton"
                                                        onclick="teamModal({{ $project['id'] }}, `{{ $project['name'] }}`, `{{ $project['project_leader_name'] }}`, {{ $project['project_leader_id'] }}, ``)"
    ...
    <div class="modal fade text-left w-100" id="detailModal" tabindex="-1" role="dialog"
        aria-labelledby="myModalLabel33" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    ...
    <div class="modal fade text-left w-100" id="teamModal" tabindex="-1" role="dialog"
        aria-labelledby="myModalLabel33" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
